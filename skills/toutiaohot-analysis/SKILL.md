...
**重要：当用户触发此 skill 时，必须严格按照以下顺序自动执行所有步骤，不要在中途停下来询问用户！**
...
3. **对每个热点执行WebSearch（关键步骤，不可跳过！）**
   - **必须**使用 WebSearch 工具，不要用其他搜索方式
   - 搜索词格式：`"{标题}" 新闻 背景 最新进展`
   - **即使已有报告，也必须重新搜索最新信息**
   - 并发搜索以提高效率（每批处理5-10个热点）
   - 记录搜索结果中的关键信息：事件脉络、参与者、影响、商业机会

4. **AI分析生成产品创意（基于最新的搜索结果）**
   - **必须基于刚才WebSearch搜索到的最新信息**，不要使用旧数据
   - 按照评分标准进行打分（有趣度80分 + 有用度20分）
   - 生成1-3个结构化的产品创意
...
**重要提醒：**
- ❌ **禁止行为**：不要在中途询问用户任何问题、不要因为发现已有文件就跳过步骤、不要在步骤3之前停下来、不要只输出分析而不生成文件、不要使用简化版的创意生成（必须用WebSearch搜索结果）
- ✅ **必须做到**：自动执行所有10个步骤、使用WebSearch工具获取最新信息、生成高质量的创意（不是模板化名称）、覆盖已有的HTML报告、完成飞书同步、归档、网站生成、Git提交
